...
//source data
import sourceData from "../translationdata/CampaignData/items.json";
...
export default function CampaignItems() {
  ...
  function onFileDropped(fileInfo, fileContent) {
    if (fileInfo.name != "items.json") {
      ToastMessage.showToast(
        "The imported filename doesn't match the expected filename: items.json"
      );
      return;
    }
  ...
  async function doWork({ language, task }) {
    ...
    try {
      ...
      if (task.getSource) {
        promises.push(
          new Promise((resolve) => {
            resolve(downloadSource("campaignitems", "English (EN)", true));
          })
        );
      }
      ...
      if (task.getTranslation) {
        promises.push(
          new Promise((resolve) => {
            resolve(downloadSource("campaignitems", language, true));
          })
        );
      }
  ...
  return (
    <CommonLayout
      ...
      projectTitle={"CAMPAIGN ITEMS"}
      onSave={() =>
        saveFile(translatedData, "items.json", (m) => ToastMessage.showToast(m))
      }
      drawerWidth={340}
